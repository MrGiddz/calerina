...
class CheckTemplateTypeAndPlan
{
    ...
    public function check(Request $request): bool
    {
        ...
        if ($plan) {
            ...
            $plan_ai_tools = $plan->getAttribute('plan_ai_tools') ?: [];
            if ($plan->getAttribute('plan_type') == 'All' && in_array($slug, $plan_ai_tools)) {
                return true;
            }
            
            if ($plan->getAttribute('plan_type') == 'Premium' && in_array($slug, $plan_ai_tools)) {
                // if ($openAi->getAttribute('premium') == 1) {
                return true;
                // }
            }
            
            if ($plan->getAttribute('plan_type') == 'Regular' && in_array($slug, $plan_ai_tools)) {
                if ($openAi->getAttribute('premium') == 0) {
                    return true;
                }
            }
