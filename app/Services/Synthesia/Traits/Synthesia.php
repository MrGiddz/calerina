...
namespace App\Services\Synthesia\Traits;
...
trait Synthesia
{
    ...
    public static function listAvatars(?string $filter = null): array
    {
        $file = Storage::disk('data')->get('synthesia/avatars.json');
        $avatars = json_decode($file, true);

        return collect($avatars)->filter(function ($avatar) use ($filter) {
            return $filter === null || $avatar['version'] === $filter || $avatar['gender'] === $filter;
        })->values()->toArray();
    }
