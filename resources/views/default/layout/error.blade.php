...
@section('content')
    <header class="fixed inset-x-0 top-0 flex justify-center border-b bg-background/10 px-4 py-6 backdrop-blur-md">
        <figure>
            @if (!empty($setting) && !empty($setting->logo_dashboard))
                <img class="max-h-14 w-auto dark:hidden"
                    src="{{ custom_theme_url($setting->logo_dashboard_path ?? '', true) }}"
                    @if (!empty($setting->logo_dashboard_2x_path)) srcset="/{{ $setting->logo_dashboard_2x_path }} 2x" @endif
                    alt="{{ $setting->site_name ?? '' }}">
                <img class="hidden max-h-14 w-auto dark:block"
                    src="{{ custom_theme_url($setting->logo_dashboard_dark_path ?? '', true) }}"
                    @if (!empty($setting->logo_dashboard_dark_2x_path)) srcset="/{{ $setting->logo_dashboard_dark_2x_path }} 2x" @endif
                    alt="{{ $setting->site_name ?? '' }}">
            @else
                <img class="max-h-14 w-auto dark:hidden" src="{{ custom_theme_url($setting->logo_path ?? '', true) }}"
                    @if (!empty($setting->logo_2x_path)) srcset="/{{ $setting->logo_2x_path }} 2x" @endif
                    alt="{{ $setting->site_name ?? '' }}">
                <img class="hidden max-h-14 w-auto dark:block"
                    src="{{ custom_theme_url($setting->logo_dark_path ?? '', true) }}"
                    @if (!empty($setting->logo_dark_2x_path)) srcset="/{{ $setting->logo_dark_2x_path }} 2x" @endif
                    alt="{{ $setting->site_name ?? '' }}">
            @endif
        </figure>
    </header>
    ...
    <div class="flex min-h-screen flex-col items-center justify-center py-12">
        <div class="mx-auto w-full pt-20 text-center lg:w-5/12">
            ...
            <div class="mx-auto sm:w-8/12">
                <x-button class="w-full" size="lg" href="{{ route('index') }}">
