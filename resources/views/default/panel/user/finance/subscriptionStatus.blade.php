...
@if ($team)
    <div class="flex flex-wrap items-center justify-between gap-y-4 text-base font-medium leading-normal">
        <div class="lg-w/5-12 w-full md:w-1/2">
            ...
            <p>
                {!! __(
                    "You have the Team plan which has a remaining balance of <strong class='font-bold'>:word</strong>
                                            words and <strong class='font-bold'>:image</strong> images. You can contact your team manager if you need
                                            more credits.",
                    [
                        'word' => Auth::user()->remaining_words,
                        'image' => Auth::user()->remaining_images,
                    ],
                ) !!}
            </p>
        </div>
                ...
                <h3 class="absolute start-1/2 top-[calc(50%-5px)] m-0 -translate-x-1/2 text-center text-xs font-normal">
                    <strong class="text-[2em] font-semibold leading-none max-sm:text-[1.5em]">
                        @if (Auth::user()->remaining_words == -1)
                            {{ __('Unlimited') }}
                        @else
                         {{ number_format(Auth::user()->remaining_words) }}
                        @endif
                    </strong>
                    <br>
                    {{ __('Tokens') }}
                </h3>
                <div class="relative [&_.apexcharts-legend-text]:!m-0 [&_.apexcharts-legend-text]:!pe-2 [&_.apexcharts-legend-text]:ps-2 [&_.apexcharts-legend-text]:!text-foreground"
                    id="chart-credit">
                </div>
            </div>
        </div>
...
@else
        ...
        @if ($setting->feature_ai_image)
            ...
            <strong class="text-heading-foreground">
                @if (Auth::user()->remaining_images == -1)
                    {{ __('Unlimited') }}
                @else
                    {{ number_format(Auth::user()->remaining_images) }}
                @endif
            </strong>
    ...
    <div class="mt-6 flex flex-wrap items-center justify-center gap-4">
        <x-button data-name="{{ \App\Enums\Introduction::SELECT_PLAN }}" class="hover:bg-primary"
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const options = {
                series: [
                    ...
                    {{ isset($total_words) ? (int) $total_words : 0 }}
                ],
